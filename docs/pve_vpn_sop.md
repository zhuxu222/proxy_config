...
- **严苛的企业安全审查 (Symantec AV)**： 目标机部署了严格的杀毒软件，执行常规的内网穿透二进制程序（如 `gost.exe`）会被 AdvML 启发式引擎和 PUA 规则直接隔离并删除。
...
```
*(保持该 PowerShell 窗口运行。如需极致隐蔽或避免误关，可考虑将此命令注册为 Windows 后台服务或放入 WSL2 Docker 容器中运行。)*
...
- **严格规范**： 地址栏必须带有 HTTP 协议头及中括号，填写为 `http://[fd3e:7070:d4dc::364]`，端口填写 `8080`。
...
1. **添加自定义代理节点**：
   ...
   - 类型选择 `HTTP`，地址填入 `fd3e:7070:d4dc::364`，端口 `8080`。命名为 `Lenovo-VPN`。

2. **创建内网策略组**：
   - 新建策略组 `Lenovo-Rules`，将 `Lenovo-VPN` 节点加入其中。

3. **编写自定义分流规则**：
   - 进入 覆写设置 -> 规则设置 -> 启用自定义规则。
   - 在规则头部添加：
...
4. **环境检查与应用**：
   - 确保 OpenClash 的 IPv6 设置 中开启了“允许 IPv6 类型 DNS 解析”。
   - 保存并应用配置。
